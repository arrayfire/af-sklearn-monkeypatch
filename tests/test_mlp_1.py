...
#from afsklearn.neural_network import MLPClassifier as afMLPClassifier

#FAILED test_mlp_1.py::test_multilabel_classification - AttributeError: 'numpy.ndarray' object has no attribute 'arr'
#FAILED test_mlp_1.py::test_multilabel_classification_af - AttributeError: 'numpy.ndarray' object has no attribute 'arr'
...
def test_fit():
    ...
    if isinstance(mlp.coefs_[0], af.Array):
        test_coefs_ = [a.to_ndarray() if a is not None else a for a in mlp.coefs_]
        test_intercepts_ = [np.squeeze(a.to_ndarray()) if a is not None else a for a in mlp.intercepts_]
        ...
        assert_almost_equal(test_coefs_[1], np.array([[0.04706], [0.154089]]).flatten(), decimal=2)
        assert_almost_equal(test_intercepts_[0], np.array([0.098333, 0.09626]), decimal=2)
        assert_almost_equal(test_intercepts_[1], np.array(0.9235), decimal=2)
        assert_almost_equal(mlp.predict_proba(X)[0, 1], 0.739, decimal=2)
    else:
        assert_almost_equal(
            mlp.coefs_[0],
            np.array([[0.098, 0.195756], [0.2956664, 0.096008], [0.4939998, -0.002244]]),
...
def test_gradient():
    ...
    for n_labels in [2, 3]:
        ...
        for activation in ACTIVATION_TYPES:
            ...
            #assert_almost_equal(numgrad, grad) #TODO! improve accuracy to 1e-7 per original epsilon
            maxdiff = np.max(np.abs(numgrad - grad))
            assert maxdiff < 2e-1

def test_gradient_af():
    Patcher.patch("mlp_classifier")
    test_gradient()
    Patcher.rollback("mlp_classifier")
...
@pytest.mark.parametrize("X,y", classification_datasets)
def test_lbfgs_classification_af(X, y):
    Patcher.patch("mlp_classifier")
    test_lbfgs_classification(X, y)
    Patcher.rollback("mlp_classifier")
...
@pytest.mark.parametrize("X,y", classification_datasets)
def test_lbfgs_classification_maxfun_af(X, y):
    Patcher.patch("mlp_classifier")
    test_lbfgs_classification_maxfun(X, y)
    Patcher.rollback("mlp_classifier")
...
def test_learning_rate_warmstart_af():
    Patcher.patch("mlp_classifier")
...
def test_multilabel_classification_af():
    Patcher.patch("mlp_classifier")
...
def test_partial_fit_classes_error():
    ...
    X = [[3, 2]]
    y = [0]
    clf = sklearn.neural_network.MLPClassifier(solver="sgd")
    clf.partial_fit(X, y, classes=[0, 1])
    with pytest.raises(ValueError):
        clf.partial_fit(X, y, classes=[1, 2])

def test_partial_fit_classes_error_af():
    Patcher.patch("mlp_classifier")
...
def test_partial_fit_classification_af():
    Patcher.patch("mlp_classifier")
...
def test_partial_fit_unseen_classes():
    ...
    clf = sklearn.neural_network.MLPClassifier(random_state=0)
    clf.partial_fit([[1], [2], [3]], ["a", "b", "c"], classes=["a", "b", "c", "d"])
    clf.partial_fit([[4]], ["d"])
    assert clf.score([[1], [2], [3], [4]], ["a", "b", "c", "d"]) > 0

def test_partial_fit_unseen_classes_af():
    Patcher.patch("mlp_classifier")
...
def test_partial_fit_errors():
    ...
    # no classes passed
    with pytest.raises(ValueError):
        sklearn.neural_network.MLPClassifier(solver="sgd").partial_fit(X, y, classes=[2])

    # lbfgs doesn't support partial_fit
    assert not hasattr(sklearn.neural_network.MLPClassifier(solver="lbfgs"), "partial_fit")

def test_partial_fit_errors_af():
    Patcher.patch("mlp_classifier")
...
def test_params_error_af(args):
    Patcher.patch("mlp_classifier")
    test_params_errors(args)
    Patcher.rollback("mlp_classifier")
...
def test_predict_proba_binary():
    ...
    clf = sklearn.neural_network.MLPClassifier(hidden_layer_sizes=5, activation="logistic", random_state=1)
    with ignore_warnings(category=ConvergenceWarning):
        clf.fit(X, y)
...
def test_predict_proba_binary_af():
    Patcher.patch("mlp_classifier")
...
def test_predict_proba_multiclass_af():
    Patcher.patch("mlp_classifier")
...
def test_predict_proba_multilabel():
    ...
    clf = sklearn.neural_network.MLPClassifier(solver="lbfgs", hidden_layer_sizes=30, random_state=0)
    clf.fit(X, Y)
    y_proba = clf.predict_proba(X)
...
def test_predict_proba_multilabel_af():
    Patcher.patch("mlp_classifier")
...
def test_sparse_matrices():
    ...
    mlp = sklearn.neural_network.MLPClassifier(solver="lbfgs", hidden_layer_sizes=15, random_state=1)
    mlp.fit(X, y)
    pred1 = mlp.predict(X)
    mlp.fit(X_sparse, y)
...
def test_tolerance():
    ...
    X = [[3, 2], [1, 6]]
    y = [1, 0]
    clf = sklearn.neural_network.MLPClassifier(tol=0.5, max_iter=3000, solver="sgd")
    clf.fit(X, y)
    assert clf.max_iter > clf.n_iter_

def test_tolerance_af():
    Patcher.patch("mlp_classifier")
    test_tolerance()
    Patcher.rollback("mlp_classifier")
...
def test_verbose_sgd():
    ...
    X = [[3, 2], [1, 6]]
    y = [1, 0]
    clf = sklearn.neural_network.MLPClassifier(solver="sgd", max_iter=2, verbose=10, hidden_layer_sizes=2)
...
def test_early_stopping():
    ...
    y = y_digits_binary[:100]
    tol = 0.2
    clf = sklearn.neural_network.MLPClassifier(tol=tol, max_iter=3000, solver="sgd", early_stopping=True)
    clf.fit(X, y)
    assert clf.max_iter > clf.n_iter_
...
def test_adaptive_learning_rate():
    X = [[3, 2], [1, 6]]
    y = [1, 0]
    clf = sklearn.neural_network.MLPClassifier(tol=0.5, max_iter=3000, solver="sgd", learning_rate="adaptive")
    clf.fit(X, y)
    assert clf.max_iter > clf.n_iter_
    assert 1e-6 > clf._optimizer.learning_rate

def test_adaptive_learning_rate_af():
    Patcher.patch("mlp_classifier")
...
def test_warm_start():
    ...
    # No error raised
    clf = sklearn.neural_network.MLPClassifier(hidden_layer_sizes=2, solver="lbfgs", warm_start=True).fit(X, y)
    clf.fit(X, y)
    clf.fit(X, y_3classes)

    for y_i in (y_2classes, y_3classes_alt, y_4classes, y_5classes):
        clf = sklearn.neural_network.MLPClassifier(hidden_layer_sizes=2, solver="lbfgs", warm_start=True).fit(
            X, y
        )
        message = (
...
def test_n_iter_no_change_inf_af():
    Patcher.patch("mlp_classifier")
...
def test_early_stopping_stratified_af():
    Patcher.patch("mlp_classifier")
    test_early_stopping_stratified()
    Patcher.rollback("mlp_classifier")


# d3m speedup inputs below

def sklearn_example(X, y) -> None:
    from sklearn.neural_network import MLPClassifier
    clf = sklearn.neural_network.MLPClassifier(
...
def sklearn_mlp(X, y) -> None:
    sklearn_example(X, y)


def afsklearn_mlp(X, y) -> None:
    Patcher.patch("mlp_classifier")
    sklearn_example(X, y)
    Patcher.rollback("mlp_classifier")
...
#@pytest.mark.skip()
def test_af_mlp_speedup():
    #assume test data in same directory as test file
    try:
        dirpath = Path().absolute().as_posix() + '/'
        print(dirpath + 'mlp_training_inputs.pickle')

        with open(dirpath + 'mlp_training_inputs.pickle', 'rb') as f:
            X = pickle.load(f)
            print(X.shape)

        with open(dirpath + 'mlp_training_output.pickle', 'rb') as f:
            y = pickle.load(f)
            print(y.shape)
    except:
        rng = np.random.RandomState(0)
        X = np.sort(rng.normal(loc=0.5, scale=0.25, size=(5000, 1)), axis=0)
        X = pd.DataFrame(data=X)
        print(X.shape)

        rng = np.random.RandomState(0)
        y = rng.normal(loc=0.5, scale=0.25, size=(5000,)).astype(np.intc)
        print(X.shape)
